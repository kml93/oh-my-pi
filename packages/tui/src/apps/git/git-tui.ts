...
class GitTuiComponent implements Component {
	...
	constructor(ui: TUI, host: GitTuiHost) {
		...
		const avatars = host.createAvatarSource(() => this.#ui.requestRender());
		...
		this.#split = new SplitPane({
			...
			divider: () => (this.#focus === "sidebar" ? theme.fg("accent", "│") : theme.fg("borderMuted", "│")),
		});
		this.#stack = new Stack({
			children: [
				{ content: (width: number) => [this.#header(width)], height: 1 },
				{ content: (width: number) => [this.#toolbar(width)], height: 1 },
	...
	#handleMouse(data: string): boolean {
		...
		return routeSgrMouseInput(data, event => {
			if (event.wheel !== null) {
				...
				const hit = this.#split.locate(event.row - 2, event.col);
				if (!hit) return true;
